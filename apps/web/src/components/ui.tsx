...
export function Modal({ title, children, onClose, wide = false }: { title: string; children: ReactNode; onClose: () => void; wide?: boolean }) {
  ...
  return <dialog ref={dialogRef} aria-labelledby={id} className={wide ? "modal wide" : "modal"} onCancel={event => { event.preventDefault(); onClose(); }}><div className="modal-head"><div><h2 id={id}>{title}</h2></div><button className="icon-button" aria-label="Close dialog" onClick={onClose}><Icon name="close"/></button></div><div className="modal-body">{children}</div></dialog>;
}
export function Table({ headings, children, count }: { headings: ReactNode[]; children: ReactNode; count?: number }) { return <div className="table-wrap"><table><thead><tr>{headings.map((heading, i) => <th key={i} scope="col">{heading}</th>)}</tr></thead><tbody>{children}</tbody></table>{count !== undefined && <div className="table-foot">{count} record{count === 1 ? "" : "s"}</div>}</div>; }
