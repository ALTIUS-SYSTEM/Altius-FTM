...
export const taskStatusSchema = z.enum(["unassigned", "assigned", "in-progress", "completed", "failed"]);
export const taskSchema = z.object({
  id: z.string(), title: z.string().trim().min(2).max(120), address: z.string().trim().min(3).max(240),
  hub: z.string(), flow: z.string(), assignee: z.string(), status: taskStatusSchema,
...
export const stateSchema = z.object({
  version: z.literal(1), tasks: z.array(taskSchema).max(2000), records: z.array(recordSchema).max(2000),
...
export const STATUS_LABELS: Record<TaskStatus, string> = { unassigned: "Unassigned", assigned: "Assigned", "in-progress": "In progress", completed: "Completed", failed: "Failed" };
