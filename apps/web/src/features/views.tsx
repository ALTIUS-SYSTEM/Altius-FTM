...
export function Tracking() {
  ...
  return <><div className="metric-grid"><Metric label="Drivers" value={roster.length} detail="On this org"/><Metric label="Completed" value={mine.filter(x => x.status === "completed").length} detail="Today"/><Metric label="Running" value={mine.filter(x => x.status === "in-progress").length} detail="Active now" tone="amber"/><Metric label="Travel time" value="2h 14m" detail="Simulated"/><Metric label="Distance" value="38.4 km" detail="Simulated"/></div>
...
export function Lhs() {
  ...
  const roster = useDriverRoster();
  const dayEvents = [
    { kind: "arrived", entity: "ALT-1041", day: today() }, { kind: "done", entity: "ALT-1041", day: today() },
    { kind: "arrived", entity: "ALT-1042", day: today() }, { kind: "done", entity: "ALT-1042", day: today() },
    { kind: "arrived", entity: "ALT-1043", day: today() }];
  const dayCosts = [{ amount: 120000, day: today() }, { amount: 35000, day: today() }, { amount: 50000, day: today() }];
  const agg = aggregateDaily(dayEvents, dayCosts, today());
  const reports = roster.slice(0, 3).map((d, i) => ({ id: `lhs-${i}`, driver: d, day: today(), ...agg, routeKm: 12.4 + i * 3, status: state.reviews[`lhs-${i}`]?.status ?? "Submitted" }));
